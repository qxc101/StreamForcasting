...
for basin_id in ids:
    ...
    # 修改时间序列长度和预测窗口为小时级数据的合适值
    batch_size = 64
    time_series_size = 480  # 更长的上下文窗口，适合小时级数据
    pred_size = 12         # 小时级预测窗口
    num_channels = 3       # 特征数量修改为3个
    ...
    model = LSTM(hidden_size=128, 
                ...
                dropout_rate=0.2, 
                bidirectional=False,
                context_window_size=time_series_size,
                pred_size=pred_size)
    
    # 不再需要这一行，因为已经在构造函数中设置了
...
# 保存结果到CSV
lstm_results.to_csv('results/lstm_hourly_results.csv')
lstm_high_results.to_csv('results/lstm_hourly_high_results.csv')
lstm_low_results.to_csv('results/lstm_hourly_low_results.csv')

# 打印汇总统计数据
print("\n=== LSTM Hourly Results Summary ===")
print("All Data:")
